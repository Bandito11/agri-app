...
export class MoonPhaseProvider {
  ...
  async getMoonPhase(date: Calendar): Promise <ApiResponse<MoonPhase>> {
    const location = await this.locationProvider.getLocation();
    const today = new Date();
    const query = `${config.URL}/moonphase/${date.year}&${date.month}&${date.day}&${location.latitude}&${location.longitude}&${today.getHours()}&${today.getMinutes()}&${today.getSeconds()}`;
